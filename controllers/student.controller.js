...
const getDepartmentProfessors = (req) =>
  User.find({
    role: "professor",
    department: req.user.department,
  }).select("name department");

const renderUploadForm = async (
  req,
  res,
  { error = null, values = {}, status = 200 } = {}
) => {
  const professors = await getDepartmentProfessors(req);

  return res.status(status).render("student/upload-single", {
    professors,
    error,
    values,
  });
...
export const uploadAssignment = async (req, res) => {
  try {
    ...
    const selectedReviewerId = reviewerId?.trim();
    const values = { title, description, category, reviewerId };
    ...
    if (selectedReviewerId) {
      if (!mongoose.Types.ObjectId.isValid(selectedReviewerId)) {
        return renderUploadForm(req, res, {
          status: 400,
          error: "Please select a valid reviewer.",
          values,
        });
      }

      const reviewer = await User.findOne({
        _id: selectedReviewerId,
        role: "professor",
        department: req.user.department,
      });
      ...
      if (!reviewer) {
        return renderUploadForm(req, res, {
          status: 400,
          error: "Please select a professor from your department.",
          values,
        });
      }

      assignmentData.reviewerId = selectedReviewerId;
    }
    ...
    return renderUploadForm(req, res, {
      status: 500,
      error: "Could not upload the assignment. Please check the form and try again.",
      values: req.body,
    });
  }
};
